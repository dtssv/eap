 ...
 * </pre>
 */
public class FreeMarkerPackageViewResolver extends org.springframework.web.servlet.view.freemarker.FreeMarkerViewResolver implements HandlerInterceptor {
	
	private String DEFAULT_VIEW_PATH_FORMAT = "/WEB-INF/classes/%s/view/%s.ftl";
	...
	protected View createView(String viewName, Locale locale) throws Exception { // viewName 必须唯一
		...
		if (StringUtil.isNotBlank(viewName) && !viewName.startsWith("/") && !viewName.startsWith(FORWARD_URL_PREFIX) && !viewName.startsWith(REDIRECT_URL_PREFIX))  {
			String viewPath = EapContext.getEnv().getProperty("app.web.viewResolver.viewPathFormat", DEFAULT_VIEW_PATH_FORMAT);
