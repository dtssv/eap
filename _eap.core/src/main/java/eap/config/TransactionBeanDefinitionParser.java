...
public class TransactionBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		for (String methodName : required) {
			...
			methodElement.setAttribute("isolation", env.getProperty("transaction.required.isolation", "READ_COMMITTED"));
			methodElement.setAttribute("rollback-for", "RollbackBizException");
		...
		for (String methodName : requiresNew) {
			...
			methodElement.setAttribute("isolation", env.getProperty("transaction.requiresNew.isolation", "READ_COMMITTED"));
			methodElement.setAttribute("rollback-for", "RollbackableBizException");
			methodElement.setAttribute("no-rollback-for", "IgnoreBizException");
			attrsElement.appendChild(methodElement);
		}
		Element methodElement = doc.createElementNS("", "method");
