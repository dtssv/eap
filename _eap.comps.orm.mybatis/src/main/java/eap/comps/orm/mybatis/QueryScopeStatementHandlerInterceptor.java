...
public class QueryScopeStatementHandlerInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		EnterpriseUserDetailsVO currUserDetailsVO = (EnterpriseUserDetailsVO) EapContext.getUserDetailsVO(); // TODO must EnterpriseUserDetailsVO
		if (currUserDetailsVO != null) {
			...
			if (sql.indexOf(qsPlaceholder) > 0) {
				List<String> roleCdList = currUserDetailsVO.getRoles();
	...
	private String filter10(UserDetailsVO currUserDetailsVO) { // 自己可见
		return currUserDetailsVO.getUserId() != null 
				? (env.getProperty("mybaits.queryScope.field.createdBy", "qst.created_by") + " = "+ currUserDetailsVO.getUserId() +"") 
					: null;
//		return "SELECT row_.* FROM (" + sql + ") row_ WHERE row_.createdBy = '"+ currUserDetailsVO.getUserNum() +"'";
	}
	private String filter20(EnterpriseUserDetailsVO currUserDetailsVO) { // 本部门可见
		return StringUtil.isNotBlank(currUserDetailsVO.getDeptCode()) 
				? (env.getProperty("mybaits.queryScope.field.deptCode", "qst.dept_code") + " = '" + currUserDetailsVO.getUserId() + "'") 
					: null;
//		return "SELECT row_.* FROM (" + sql + ") row_ WHERE row_.deptCode = '" + currUserDetailsVO.getDeptCd() + "'";
	}
	private String filter21(EnterpriseUserDetailsVO currUserDetailsVO) { // 下级部门可见
		return StringUtil.isNotBlank(currUserDetailsVO.getDeptCode()) 
				? (env.getProperty("mybaits.queryScope.field.deptCode", "qst.dept_code") + " LIKE '"+ currUserDetailsVO.getUserId() +"%'") 
					: null;
//		return "SELECT row_.* FROM (" + sql + ") row_ WHERE row_.deptCode LIKE '"+ currUserDetailsVO.getDeptCd() +"%'";
	}
	private String filter30(EnterpriseUserDetailsVO currUserDetailsVO) { // 本机构可见
		return StringUtil.isNotBlank(currUserDetailsVO.getOrgCode()) 
				? (env.getProperty("mybaits.queryScope.field.orgCode", "qst.org_code")+ " = '" + currUserDetailsVO.getUserId() + "'") 
					: null;
//		return "SELECT row_.* FROM (" + sql + ") row_ WHERE row_.orgCode = '" + currUserDetailsVO.getOrgCd() + "'";
	}
	private String filter31(EnterpriseUserDetailsVO currUserDetailsVO) { // 下级机构可见
		return StringUtil.isNotBlank(currUserDetailsVO.getOrgCode()) 
				? (env.getProperty("mybaits.queryScope.field.orgCode", "qst.org_code")+ " LIKE '" + currUserDetailsVO.getUserId() + "%'")
					: null;
